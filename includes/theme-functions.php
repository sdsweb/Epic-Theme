		...
		<div class="sds-sitemap sitemap">
			<?php if ( apply_filters( 'sds_sitemap_show_pages', true ) ) : // Allow pages to not be displayed ?>
				<div class="sitemap-pages page-list">
					<h2 title="<?php esc_attr_e( 'Pages', 'epic' ); ?>"><?php _e( 'Pages', 'epic' ); ?></h2>
					<ul>
			...
			<?php if ( apply_filters( 'sds_sitemap_show_monthly_archives', true ) ) : // Allow monthly archives to not be displayed ?>
				<div class="sitemap-archives sitemap-monthly-archives monthly-archives archive-list">
					<h2 title="<?php esc_attr_e( 'Monthly Archives', 'epic' ); ?>"><?php _e( 'Monthly Archives', 'epic' ); ?></h2>
					<ul>
			...
			<?php if ( apply_filters( 'sds_sitemap_show_categories', true ) ) : // Allow categories to not be displayed ?>
				<div class="sitemap-categories category-list">
					<h2 title="<?php esc_attr_e( 'Blog Categories', 'epic' ); ?>"><?php _e( 'Blog Categories', 'epic' ); ?></h2>
					<ul>
...
if ( ! function_exists( 'sds_archive_title' ) ) {
	function sds_archive_title() {
		...
		// Otherwise use fallback functionality
			...
			?>
				<h1 title="<?php esc_attr_e( 'Author Archive:', 'epic' ); ?> <?php echo ( $author instanceof WP_User ) ? $author->display_name : false; ?>" class="page-title author-archive-title">
					<?php _e( 'Author Archive:', 'epic' ); ?> <?php echo ( $author instanceof WP_User ) ? $author->display_name : false; ?>
				</h1>
			<?php
			// Categories
			elseif ( is_category() ) :
			?>
				<h1 title="<?php single_cat_title( __( 'Category Archive: ', 'epic' ) ); ?>" class="page-title category-archive-title">
					<?php single_cat_title( __( 'Category Archive: ', 'epic' ) ); ?>
				</h1>
			<?php
			// Tags
			elseif ( is_tag() ) :
			?>
				<h1 title="<?php single_tag_title( __( 'Tag Archive: ', 'epic' ) ); ?>" class="page-title tag-archive-title">
					<?php single_tag_title( __( 'Tag Archive: ', 'epic' ) ); ?>
				</h1>
			<?php
			// Daily Archives
			elseif ( is_day() ) :
			...
			?>
				<h1 title="<?php esc_attr_e( 'Daily Archives:', 'epic' ); ?> <?php echo $the_date; ?>" class="page-title day-archive-title daily-archive-title">
					<?php _e( 'Daily Archives:', 'epic' ); ?> <?php echo $the_date; ?>
				</h1>
			<?php
			// Monthly Archives
			elseif ( is_month() ) :
			...
			?>
				<h1 title="<?php esc_attr_e( 'Monthly Archives:', 'epic' ); ?> <?php echo $the_date; ?>" class="page-title month-archive-title monthly-archive-title">
					<?php _e( 'Monthly Archives:', 'epic' ); ?> <?php echo $the_date; ?>
				</h1>
			<?php
			// Yearly Archives
			elseif ( is_year() ) :
			...
			?>
				<h1 title="<?php esc_attr_e( 'Yearly Archives:', 'epic' ); ?> <?php echo $the_date; ?>" class="page-title year-archive-title yearly-archive-title">
					<?php _e( 'Yearly Archives:', 'epic' ); ?> <?php echo $the_date; ?>
				</h1>
			<?php
			endif;
		endif;
...
if ( ! function_exists( 'sds_no_posts' ) ) {
	...
	?>
		<section class="no-results no-posts">
			<p><?php _e( 'We were not able to find any posts. Please try again.', 'epic' ); ?></p>
		</section>
	<?php
	}
}
...
if ( ! function_exists( 'sds_post_meta' ) ) {
	function sds_post_meta() {
		...
		?>
			<p>
			<?php
				printf( __( 'This entry was tagged in %1$s.', 'epic' ),
				get_the_tag_list( '', ', ' ) );
			?>
			</p>
		<?php
...
if ( ! function_exists( 'sds_comment' ) ) {
	function sds_comment( $comment, $args, $depth ) {
		...
		<li id="comment-<?php comment_ID(); ?>" <?php comment_class(); ?>>
			<p><?php _e( 'Pingback:', 'epic' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( 'Edit', '<span class="ping-meta"><span class="edit-link">', '</span></span>' ); ?></p>
		</li>
		<?php
			break;
			default :
			...
			<article id="comment-<?php comment_ID(); ?>" <?php comment_class(); ?>>
				<section class="comment-author vcard">
					<section class="author-details">
						...
						<header class="comment-meta">
							<cite class="fn">
								<?php
									printf( __( '<a href="%1$s"><time datetime="%2$s" itemprop="commentTime">%3$s</time></a>', 'epic' ),
										esc_url( get_comment_link( $comment->comment_ID ) ),
										get_comment_time( 'c' ),
										sprintf( __( '%1$s at %2$s', 'epic' ), get_comment_date(), get_comment_time() )
									);
								?>

								<?php edit_comment_link( __( 'Edit', 'epic' ), '<span class="edit-link">', '<span>' ); ?>
							</cite>
						</header>
					</section>
				</section>
				...
				<section class="comment-content-container">
					<?php if ( $comment->comment_approved == '0' ) : ?>
						<p class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'epic' ); ?></p>
					<?php endif; ?>
				...
				<section class="reply">
					<?php comment_reply_link( array_merge( $args, array( 'reply_text' => __( 'Reply', 'epic' ), 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
				</section>
			</article>
		</li>
		<?php
...
function sds_customize_register( $wp_customize ) {
	...
	$wp_customize->get_section( 'title_tagline' )->title = __( 'Logo/Site Title & Tagline', 'epic' );
...
function sds_after_setup_theme() {
	...
	register_nav_menus( array(
		'top_nav' => __( 'Top Navigation', 'epic' ),
		'primary_nav' => __( 'Primary Navigation', 'epic' ),
		'footer_nav' => __( 'Footer Navigation', 'epic' )
	) );
}
...
function sds_widgets_init() {
	...
	// Secondary sidebar
	register_sidebar( array(
		'name'          => __( 'Secondary Sidebar', 'epic' ),
		'id'            => 'secondary-sidebar',
		'description'   => __( 'This widget area is the secondary widget area.', 'epic' ),
	...
	register_sidebar( array(
		...
		'description'   => __( '*This widget area is only displayed if a Front Page is selected via Settings > Reading in the Dashboard.* This widget area is displayed above the content on the Front Page.', 'epic' ),
	...
	register_sidebar( array(
		'name'          => __( 'Header Call To Action', 'epic' ),
