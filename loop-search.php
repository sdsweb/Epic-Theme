...
?>
	<header class="search-title">
		<h1 title="<?php esc_attr_e( sprintf( __( 'Search results for \'%s\'', 'epic' ), get_search_query() ) ); ?>" class="page-title"><?php printf( __( 'Search results for "%s"', 'epic' ), get_search_query() ); ?></h1>
	</header>
	...
	<?php while ( have_posts() ) : the_post(); ?>
		<section id="post-<?php the_ID(); ?>" <?php post_class( 'latest-post cf' ); ?>>
			...
			<?php the_content( __( 'Continue Reading', 'epic' ) ); ?>
		</section>
	<?php endwhile; ?>
<?php else : // No search results ?>
	<header class="search-title">
		<h1 title="<?php esc_attr_e( sprintf( __( 'No results for \'%s\'', 'epic' ), get_search_query() ) ); ?>'" class="page-title"><?php printf( __( 'No results for "%s"', 'epic' ), get_search_query() ); ?></h1>
	</header>
	...
	<section class="no-results no-posts no-search-results latest-post">
		...
		<section id="search-again" class="search-again search-block no-posts no-search-results">
			<p><?php _e( 'Would you like to search again?', 'epic' ); ?></p>
